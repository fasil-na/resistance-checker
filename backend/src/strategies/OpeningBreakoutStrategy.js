...
export class OpeningBreakoutStrategy {
    ...
    run(candles, params) {
        if (candles.length < 3)
            return [];
        const { rangeHour = 4, rangeMinute = 0, cutoffHour = 2, cutoffMinute = 15, buffer = 2, riskReward = 1.2, useTrailingSL = false, resolution = '15' } = params;
        let trades = [];
        let currentTrade = null;
        let today = null;
        let rangeHigh = null;
        let rangeLow = null;
        let breakCandleHigh = null;
        let breakCandleLow = null;
        let waitingForWickBreak = false;
        let breakoutDirection = null;
        let breakoutTimeStr = null;
        ...
        for (let i = 2; i < candles.length; i++) {
            ...
            if (!waitingForWickBreak) {
                ...
                if (brkRange === 0 || (brkBody / brkRange) < 0.5)
                    continue;
...
//# sourceMappingURL=OpeningBreakoutStrategy.js.map